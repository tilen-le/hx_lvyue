package com.hexing.workflow.utils;
...
import com.hexing.workflow.common.constant.ActConstant;
import com.hexing.workflow.common.enums.BusinessStatusEnum;
import com.hexing.workflow.domain.*;
import com.hexing.workflow.domain.bo.SendMessage;
import com.hexing.workflow.domain.bo.TaskCompleteBo;
import com.hexing.workflow.domain.vo.ActBusinessRuleVo;
import com.hexing.workflow.domain.vo.MultiVo;
import com.hexing.workflow.domain.vo.ProcessNode;
import com.hexing.workflow.service.*;
import com.hexing.workflow.activiti.cmd.DeleteExecutionCmd;
import com.hexing.workflow.activiti.cmd.DeleteTaskCmd;
import com.hexing.workflow.activiti.cmd.ExpressCmd;
...
public class WorkFlowUtils {
    ...
    public static void setStatusFileValue(Object obj, String id) {
        ...
        try {
            businessStatus = claszz.getDeclaredField(ActConstant.ACT_BUSINESS_STATUS);
            businessStatus.setAccessible(true);
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
            throw new ServiceException("未找到" + ActConstant.ACT_BUSINESS_STATUS + "属性：" + e.getMessage());
        }
        try {
    ...
    public static void setProcessInstIdFileValue(Object obj, String id) {
        ...
        try {
            processInstanceId = claszz.getDeclaredField(ActConstant.PROCESS_INSTANCE_ID);
            processInstanceId.setAccessible(true);
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
            throw new ServiceException("未找到" + ActConstant.PROCESS_INSTANCE_ID + "属性：" + e.getMessage());
        }
        try {
    ...
    public static List<Long> getAssigneeIdList(String params, String chooseWay, String nodeName) {
        ...
        // 按用户id查询
        if (ActConstant.WORKFLOW_PERSON.equals(chooseWay)) {
            queryWrapper.in(SysUser::getUserId, paramList);
            list = sysUserMapper.selectList(queryWrapper);
            //按角色id查询用户
        } else if (ActConstant.WORKFLOW_ROLE.equals(chooseWay)) {
            ...
            //按部门id查询用户
        } else if (ActConstant.WORKFLOW_DEPT.equals(chooseWay)) {
